...
describe('OAuth2 Async Module Use Factory Controller (e2e)', () => {
  ...
  describe('POST /oauth2/token "client_credentials"', () => {
    beforeEach(async () => {
      const fixturesLoader: FixturesLoaderService = app.get<
        FixturesLoaderService
      >(FixturesLoaderService);
      await fixturesLoader.loadFixtures(
        __dirname + '/fixtures/client-credentials',
      );
    });

    it.each([
      [
        '6ab1cfab-0b3d-418b-8ca2-94d98663fb6f',
      ...
      [
        'f9f9f9ef-34b3-428e-a742-669aecd6c889',
        '4Xg6JgvWfmIT3P5cCev2wehH8sWD3lrd',
        ['app-1'],
      ],
      [
        '051d6291-2ba7-4dd9-8a18-b590b4a9a457',
        'YLbvzkTRG40SKMm5DMfoWZD3BRZCV5Dq',
        ['app-3'],
      ],
    ])(
      'Should authenticate client (%s, %s, "[%s]")',
    ...
    it.each([
      ...
      'Should authenticate client without expiration (%s, %s, "[%s]")',
      (clientId, clientSecret, scopes) => {
        return request(app.getHttpServer())
          .post('/oauth2/token')
          .query({
    ...
    it.each([
      ...
      'Fails when scope is invalid (%s, %s, "[%s]")',
      (clientId, clientSecret, scopes) => {
        return request(app.getHttpServer())
          .post('/oauth2/token')
          .query({
    ...
    it.each([['unkown', '6nV9GGm1pu8OY0HDZ3Y7QsVnxtkb60wu', ['app-1']]])(
      'Fails the client is unknown or the secret is invalid (%s, %s, "[%s]")',
      (clientId, clientSecret, scopes) => {
        return request(app.getHttpServer())
          .post('/oauth2/token')
          .query({
  ...
  describe('POST /oauth2/token "refresh_token"', () => {
    ...
    it.each([
      [
        'TYUIKNBVGSZ345678IUJHGVHJKL',
        '6ab1cfab-0b3d-418b-8ca2-94d98663fb6f',
        '6nV9GGm1pu8OY0HDZ3Y7QsVnxtkb60wu',
      ],
      [
        'TYUIOLNBVFTYUJNBVGHYUIL',
        'f9f9f9ef-34b3-428e-a742-669aecd6c889',
        '4Xg6JgvWfmIT3P5cCev2wehH8sWD3lrd',
      ],
    ])(
      'Should renew using the refresh token (%s)',
    ...
    it.each([
      ...
      'Should reject past valid or unkown refresh token renewal (%s)',
      (refreshToken, clientId, clientSecret) => {
        return request(app.getHttpServer())
          .post('/oauth2/token')
          .query({
  ...
  describe('AuthGuard(bearer) should secure routes using access tokens', () => {
    ...
    it.each([
      ...
      'Reject calls with invalid access tokens (%s)',
      (accessToken: string) => {
        return request(app.getHttpServer())
  ...
  describe('POST /oauth2/token "password"', () => {
    ...
    it.each([
      ...
      'Should authenticate the user with the client (%s, %s, "[%s]", %s, %s)',
